...
## 6. The cell widths, counted: equivalent to factoring

The elliptic cells `r_p`, `r_q` divide `#E(F_p)` and `#E(F_q)`, and `#E(Z/N) = #E(F_p) #E(F_q)`. An oracle for that one number factors `N`: `[#E(Z/N)] P = O` modulo both primes, and removing a small prime `l` that divides only one of the two orders leaves a multiple that vanishes modulo one prime and not the other.

With the count supplied by brute force as the oracle, one count per number factored **40 of 40** semiprimes; the prime removed was at most 2111. The reverse direction -- factoring gives the count -- is Schoof's algorithm modulo each prime, so counting points on `E(Z/N)` is equivalent to factoring `N` (Kunihiro and Koyama, 1998). It joins `phi(N)`, `tau(N)` and `r_4(N)` on the list of quantities that carry the factorisation openly and are polynomial-time equivalent to it.
_Generated by `experiments/exp31_elliptic_pascal.py` in 117.4s._
